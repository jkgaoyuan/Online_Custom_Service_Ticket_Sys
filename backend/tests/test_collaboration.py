...
async def test_transfer_ticket_success_200(client, agent_auth_headers, db):
...
async def test_transfer_creates_collaboration_record_200(client, agent_auth_headers, admin_auth_headers, db):
    agent = await db.execute(select(User).where(User.username == "agent_test"))
    agent = agent.scalar_one()
...
async def test_request_assistance_success_201(client, agent_auth_headers, db):
    agent = await db.execute(select(User).where(User.username == "agent_test"))
    agent = agent.scalar_one()
...
async def test_transfer_notification_sent_200(client, agent_auth_headers, db):
    agent = await db.execute(select(User).where(User.username == "agent_test"))
    agent = agent.scalar_one()
...
async def test_transfer_open_ticket_status_change_200(client, agent_auth_headers, db):
    agent = await db.execute(select(User).where(User.username == "agent_test"))
    agent = agent.scalar_one()
